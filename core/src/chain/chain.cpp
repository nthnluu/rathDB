...
Chain::Chain() {
    ...
    std::unique_ptr<FileInfo> undoBlockFileInfo = _chain_writer->write_undo_block(UndoBlock::serialize(genesis_undo_block));

    // Write the genesis block to disk
    std::unique_ptr<FileInfo> genesisBlockFileInfo = _chain_writer->write_block(Block::serialize(*_active_chain_last_block));

    // Construct block record
    BlockRecord blockRecord = new BlockRecord(*_active_chain_last_block->block_header, NULL, NULL, genesisBlockFileInfo, undoBlockFileInfo);

    // Store the genesis block in the coin_database
    _block_info_database->store_block_record(RathCrypto::hash(Block::serialize(*_active_chain_last_block)), blockRecord);
}
...
std::unique_ptr<Block> Chain::get_block(uint32_t block_hash) {
    ...
    if (blockRecord) {
        FileInfo fileInfo = FileInfo(blockRecord->block_file_stored, blockRecord->block_offset_start, blockRecord->block_offset_end);
...
std::unique_ptr<Block> Chain::get_last_block() {
    ...
    std::unique_ptr<BlockHeader> blockHeader = std::make_unique<BlockHeader>(_active_chain_last_block->block_header->version,
    ...
    for (std::unique_ptr<Transaction>& transaction : _active_chain_last_block->transactions) {
        ...
        for (std::unique_ptr<TransactionInput>& txInput : transaction->transaction_inputs) {
            transactionInputs.push_back(std::make_unique<TransactionInput>(txInput->reference_transaction_hash, txInput->utxo_index, txInput->signature));
        }
        ...
        transactions.push_back(std::make_unique<Transaction>(transactionInputs, transactionOutputs, transaction->version, transaction->lock_time));
    }
